...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6285732415700@c.us'
...
//--------------------------------------------------------------------------------------------------------------//			
        ...
        case prefix+'resetlinkgroup':
            ...
            break
        case prefix+'getses':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner er', id)            
            const sesPic = await tobz.getSnapshot()
            tobz.sendFile(from, sesPic, 'session.png', 'testing', id)
            break
        case prefix+'admin':
            let admn = `This is list of Admin\nTotal : ${adminNumber.length}\n`
